...
We also establish a new evaluation protocol based on temporal IoU and event-level F1 metrics, revealing a significant gap between frame-level accuracy and event-level performance across standard VAD benchmarks.
...
<figure>
  ...
  <figcaption><sub><b>Figure 2:</b>Dual branch event-level anomaly detection framework. Given an input pose sequence, the model processes the data through two
